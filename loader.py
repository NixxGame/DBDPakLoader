...
class DBDModLoader(_BaseClass):
    ...
    def import_mod_folder(self):
        ...
        self.load_mods()
        if errors:
            self.after(50, lambda: self.toast(
                "Import failed:\n" + "\n".join(errors[:2])
                + ("..." if len(errors) > 2 else ""),
                mode="error", duration=6000))
        elif len(paths) > 1:
            self.after(50, lambda n=len(paths): self.toast(
                f"Imported {n} mods", mode="success"))
    ...
    def _do_import(self, path, silent=False):
        ...
        if p.is_dir():
            ...
            if not silent:
                self.load_mods()
                self.after(50, lambda n=name: self.toast(f"Imported '{n}'", mode="success"))
            return
    ...
    def _on_drop(self, event):
        ...
        for p in paths:
            try:
                self._do_import(p, silent=True)   # silent=True — we show one summary toast
                imported += 1
                last_name = Path(p).stem
            except Exception as e:
                errors += 1
                self.after(50, lambda err=str(e), nm=Path(p).name:
                    self.toast(f"Failed: {nm}\n{err}", mode="error", duration=5000))

        self.load_mods()
        if imported == 1 and not errors:
            self.status_var.set(f"✅ Imported: {last_name}")
            self.after(50, lambda n=last_name: self.toast(f"Imported '{n}'", mode="success"))
        elif imported > 1:
            self.status_var.set(f"✅ Imported {imported} mod(s)")
            self.after(50, lambda n=imported, e=errors: self.toast(
                f"Imported {n} mod(s)" + (f"  •  {e} failed" if e else ""),
                mode="success" if not e else "warn"))
